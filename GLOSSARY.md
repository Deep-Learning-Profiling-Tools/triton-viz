...
- src/op_workspace.ts: op tabs, workspace layout, and code peek panel.
- src/tensor_view.ts: 3D tensor rendering and histogram overlay integration.
- src/dimension_utils.ts: CAD-style dimension lines and vector text.
- src/state.ts: shared active program and toggle state.
- src/logger.ts: action and info logging.

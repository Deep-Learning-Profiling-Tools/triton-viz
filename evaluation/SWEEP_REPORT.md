...
Pre-exit representative (await-collapse repair, rows 56→59, TN 24→25, witness 25/25→27/27): the await abstraction's single kept poll silently LOST races living only on the spin's dropped failed iterations whenever the conflicting access keeps the closed world intact. The encoder now emits a value-model-free representative alongside every awaited poll, carrying the failed iterations' footprint and modes (identity-RMW polls keep the write half; a failed CAS writes nothing; plain-load polls stay plain reads) with no rf/sw edge in or out: any unrolled race on a failed iteration survives on the representative with no more ordering, the over-report direction. Two planted corners that previously PROVED silently now report with witness-matched `:pre-exit` endpoints: `trb016_pc_wait_cta_reset_yes` (a value-modeled equal-width relaxed cta-scoped xchg reset po-before the gpu publish — the modeled written value opens no escape and the termination premise pins the poll to the publisher) and `trb016_pc_wait_flag_read_yes` (a plain read of the awaited flag vs the failed iterations' identity write-backs). The morally-strong twin `trb016_pc_wait_atomic_reset_no` keeps its conditional proof (mutants: pid_pin / sem_relax flip, atomic_to_store degrades at spin-shape). Re-baselined seven-corpus sweep: ZERO verdict flips; the three weak-access racy twins that already reported (`trb017_mutex_plain_unlock_yes`, `trb016_pc_wait_cta_scope_yes`, `trb018_lookback_cta_yes`) each gain exactly one report pairing the weak access with the representative, and no race-free row changes.

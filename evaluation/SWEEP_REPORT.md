...
| torchao ×8 races-unclassified | wrapper-coupled any-grid (§3 ¹) | witness pids out of launch extent on all 8 (attnq single_reduce ×3, deepgemm weight-quant ×2, kernel blockwise weight-quant ×2, common split-k matmul ×1) |
...
| carried-value `scf.while` (spin: `mm_streamk`; plain iteration: torchao mx swizzles) | flaggems 1 + torchao 2 | S6 await-abstraction extension; the torchao pair shows the gate also catches NON-spin carried whiles |

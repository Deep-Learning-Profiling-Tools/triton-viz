...
def test_tensor_calibration_surface_uses_dtype_specific_throughput(tmp_path):
    ...
    def active_seconds(dtype, flops):
        startup_ns = 1000.0 if dtype == "float32" else 500.0
        flops_per_ns = 20_000.0 if dtype == "float32" else 80_000.0
        return (startup_ns + flops / flops_per_ns) * 1e-9

    with path.open("w", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=fields)
        writer.writeheader()
        for dtype in ("float32", "bfloat16"):
            for flops in (1_000_000, 2_000_000):
                writer.writerow({
                    "row_type": "benchmark",
                    "status": "ok",
                    "kind": "tensor_matmul",
                    ...
                    "profile.tensor_engine_active_time": active_seconds(
                        dtype, flops
                    ),
                })

    calibration = TensorCalibrationSurface.from_csv(path)
    ...
    assert calibration.flops_per_ns("float16") == pytest.approx(20_000.0)
    with pytest.raises(ValueError, match="Missing exact TensorE calibration"):
        calibration.flops_per_ns("float16", strict=True)

    model = CostModel(tensor_calibration=calibration, tensor_startup_ns=0)
    dot = {
        "op": "dot",
        "engine": "tensor",
        "flops": 2 * 128 * 512 * 128,
    ...
    assert model.cost_ns(dot) == pytest.approx(dot["flops"] / 80_000.0)

    result = simulate([dict(dot, seq=1)], model)
    assert result.engine_busy_ns["tensor"] == pytest.approx(
        dot["flops"] / 80_000.0 + 500.0
    )
...
def test_tensor_calibration_csv_rejects_empty(tmp_path):
    path = tmp_path / "tensor.csv"
    path.write_text(
        "row_type,status,kind,spec.dtype,derived.tensor_tflops_active\n"
        "benchmark,ok,tensor_matmul,float32,\n"
    )
    with pytest.raises(ValueError, match="No tensor_matmul calibration rows"):
        TensorCalibrationSurface.from_csv(path)


def test_tensor_calibration_median_aggregates_intentional_mode_rows(tmp_path):
    path = tmp_path / "tensor.csv"
    fields = [
        "row_type",
        "status",
    ...
    with path.open("w", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=fields)
        writer.writeheader()
        for mode, flops, active_ns in (
            ("dependent_accumulate", 1_000_000, 100.0),
            ("independent_stream", 1_000_000, 200.0),
            ("dependent_accumulate", 2_000_000, 200.0),
            ("independent_stream", 2_000_000, 300.0),
        ):
            writer.writerow({
                "row_type": "benchmark",
                "status": "ok",
    ...
    calibration = TensorCalibrationSurface.from_csv(path)
    assert calibration.flops_per_ns("float32") == pytest.approx(10_000.0)
    assert calibration.startup_ns("float32") == pytest.approx(50.0)


def test_tensor_calibration_from_tiled_control_csv_fits_effective_per_dot(tmp_path):
    ...
    with path.open("w", newline="") as file:
        ...
        for dtype, points in (
            ...
            for dot_count, active_us in points:
                writer.writerow({
                    "row_type": "benchmark",
                    "status": "ok",
                    "kind": "tensor_matmul_tiled",
                    "spec.dtype": dtype,
                    "work.dot_count": dot_count,
                    "work.matmul_flops": dot_count * 16_000_000,
                    "profile.tensor_engine_active_time": active_us * 1e-6,
                })
    calibration = TensorCalibrationSurface.from_csv(
        path, benchmark_name="tensor_matmul_tiled"
    )
    assert calibration.ns_per_dot("float32") == pytest.approx(1250.0)
    assert calibration.ns_per_dot("bf16") == pytest.approx(312.5)


def test_tensor_calibration_from_small_dot_control_is_shape_keyed(tmp_path):
    path = tmp_path / "all_results.csv"
    fields = [
        "row_type",
        "status",
    ...
    with path.open("w", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=fields)
        writer.writeheader()
        # (m, n) = (64, 128): 1000 ns per dot + 2000 ns per-kernel startup.
        # (m, n) = (128, 128): 800 ns per dot + 1000 ns per-kernel startup.
        for m, n, slope_ns, intercept_ns in (
            (64, 128, 1000.0, 2000.0),
            (128, 128, 800.0, 1000.0),
        ):
            for dot_count in (1, 2, 4):
                active_ns = intercept_ns + dot_count * slope_ns
                writer.writerow({
                    "row_type": "benchmark",
                    "status": "ok",
                    "kind": "tensor_matmul_small",
                    "spec.dtype": "float32",
                    "spec.m": m,
                    "spec.n": n,
                    "spec.k": 128,
                    "work.dot_count": dot_count,
                    "work.matmul_flops": 2 * m * n * 128 * dot_count,
                    "profile.tensor_engine_active_time": active_ns * 1e-9,
                })

    calibration = TensorCalibrationSurface.from_csv(
        path, benchmark_name="tensor_matmul_small"
    )
    assert calibration.ns_per_dot("float32", m=64, n=128) == pytest.approx(1000.0)
    assert calibration.ns_per_dot("float32", m=128, n=128) == pytest.approx(800.0)
    assert calibration.startup_ns("float32", m=64, n=128) == pytest.approx(2000.0)
    assert calibration.ns_per_dot("float32", m=64, n=128, strict=True) == pytest.approx(
        1000.0
    )
    with pytest.raises(ValueError, match="shape=\\(32,32\\)"):
        calibration.ns_per_dot("float32", m=32, n=32, strict=True)
    ...
    assert model.cost_ns(dot) == pytest.approx(1000.0)


def test_tensor_calibration_operator_csv_is_not_accepted_as_calibration(tmp_path):
    """Holdout operator results must never become TensorE calibration."""
        ...
        for dtype, points in (
            ...
            for dot_count, active_us in points:
                writer.writerow({
                    ...
                    "status": "ok",
                    "dot_count": dot_count,
                    "hardware_tensor_active_us": active_us,
                })
    with pytest.raises(ValueError, match="No tensor_matmul calibration rows"):
        TensorCalibrationSurface.from_csv(path)

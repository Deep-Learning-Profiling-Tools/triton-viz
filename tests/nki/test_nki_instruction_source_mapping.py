...
def test_source_region_id_is_stable_and_separates_regions():
    ...
    _annotate_fusion_signature(copy)
...
def test_parse_penguin_and_transfer_bounded_region_assignment(tmp_path):
    from triton_viz.tools.nki_instruction_source_mapping import (
        assign_penguin_regions,
        parse_penguin,
    )

    penguin = tmp_path / "penguin.py"
    penguin.write_text(
        "\n".join(  # noqa: FLY002 - fixture lines are clearer as a list
            [
    ...
    assert assign_penguin_regions(
        ops, [{"tokens": ["multiply", "reduce_sum"]}, {"tokens": ["multiply"]}]
    ) == {2: 0, 3: 0, 5: 1}


def test_where_region_is_activation_table_owner_without_penguin_activation_op():
    from triton_viz.tools.nki_instruction_source_mapping import activation_owner_regions

    regions = [{"tokens": ["add"]}, {"tokens": ["where", "multiply"]}]
    assert activation_owner_regions({}, {}, regions) == {1}

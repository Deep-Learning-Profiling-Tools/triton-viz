...
def test_pipeline_dry_run_keeps_controls_and_holdouts_separate(tmp_path, capsys):
    ...
    assert "/holdouts/attention_bf16_v1" in output
...
def test_pipeline_fit_and_evaluate_dry_run_use_dtype_specific_dma(tmp_path, capsys):
    ...
    assert "--tensor-calibration-csv" in evaluate_output
    ...
    assert evaluate_output.count("--strict-calibration") == 17
...
def test_formal_split_is_exactly_the_documented_35_cases():
    ...
    assert _split_case_count(data["splits"]["attention_fp32_v1"]) == 9
    assert _split_case_count(data["splits"]["attention_bf16_v1"]) == 9

...
def test_masked_load():
    ...
    print(f"Input array:\n{arr}")
    print(f"Mask:\n{mask}")
    print("Keys: (slice(0, 2), slice(0, 2))")
    print(f"Result:\n{result}")
    print(f"Expected:\n{expected}")
    print(f"Match: {np.array_equal(result, expected)}\n")
    ...
    print(f"Input array:\n{arr}")
    print("Keys: (slice(0, 5), slice(0, 3)) - exceeds array bounds")
    ...
    result = masked_load(
        arr, (slice(0, 4), 2), mask=mask
    )  # Get column 2, allow extending rows
    print(f"Input array:\n{arr}")
    print("Keys: (slice(0, 4), 2) - slice extends beyond array rows")
    print(f"Result: {result}")
    print(f"Result shape: {result.shape}")
    ...
    try:
        result = masked_load(arr, arr_slice, mask=mask)
        raise RuntimeError(
            "Should've raised an assertion error (arr[arr_slice].shape != mask.shape) but did not"
        )
...
def test_masked_store():
    ...
    try:
        ...
        print("ERROR: Should have raised AssertionError but didn't")
        print(f"values.shape: {values.shape}")
        print(f"mask.shape: {mask.shape}")
        raise RuntimeError("Should've raised an assertion error but did not")
    except AssertionError:
        print(
            "Success: Correctly raised AssertionError for values/mask shape mismatch\n"
        )

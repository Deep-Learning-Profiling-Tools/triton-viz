...
def print_op_details(test_name, operation_type, input_data, keys, values=None, mask=None, output=None, error_expected=None):
...
def test_masked_load_none_case():
    ...
    print_op_details("Test 1: mask=None case", "load", arr, (slice(1, 4),), mask=None, output=result)
...
def test_masked_load_in_bounds():
    ...
    result[[0,1,2],[2,1,0]] = 0  # manually set masked out positions to 0 for testing
    expected = np.array([[1, 2, 0], [4, 0, 6], [0, 8, 9]])

    print_op_details("Test 2: In-bounds indexing with mask", "load", arr, (slice(0, 3), slice(0, 3)), mask=mask, output=result)
...
def test_masked_load_out_of_bounds():
    ...
    print_op_details("Test 3: Out-of-bounds indexing with mask", "load", arr, (slice(0, 5), slice(0, 3)), mask=mask, output=result)
...
def test_masked_load_integer_oob():
    ...
    print_op_details("Test 4: Integer indexing out of bounds", "load", arr, ([5],), mask=mask, output=result)
...
def test_masked_load_array_indexing():
    ...
    print_op_details("Test 5: Array indexing", "load", arr, (indices,), mask=mask, output=result)
...
def test_masked_load_mixed_indexing():
    ...
    print_op_details("Test 6: Mixed indexing types", "load", arr, (slice(0, 4), 2), mask=mask, output=result)
...
def test_masked_load_complex_indexing():
    ...
    print("Load Test 7: Complex multi-dimensional indexing (omitted details, check code to see inputs/outputs)")
...
def test_masked_load_incomplete_slices():
    ...
    print_op_details("Test 8: Incomplete slices", "load", arr, arr_slice, mask=mask, output=result)
...
def test_masked_load_index_error_with_true_mask():
    ...
    print_op_details("Test 9: Index error should still happen when mask=True at OOB location", "load", arr, arr_slice, mask=mask, error_expected="IndexError")
...
def test_masked_load_mask_shape_mismatch():
    ...
    print_op_details("Test 10: Error if mask shape wrong", "load", arr, arr_slice, mask=mask, error_expected="AssertionError")
...
def test_masked_store_none_case():
    ...
    print_op_details("Store Test 1: mask=None case", "store", arr, (slice(1, 4),), values=values, mask=None, output=arr_copy)
...
def test_masked_store_in_bounds():
    ...
    print_op_details("Store Test 2: In-bounds indexing with mask", "store", arr, (slice(0, 3), slice(0, 3)), values=values, mask=mask, output=arr_copy)
...
def test_masked_store_out_of_bounds():
    ...
    print_op_details("Store Test 3: Out-of-bounds indexing with mask", "store", arr, (slice(0, 5), slice(0, 3)), values=values, mask=mask, output=arr_copy)
...
def test_masked_store_integer_oob():
    ...
    print_op_details("Store Test 4: Integer indexing out of bounds", "store", arr, ([5],), values=values, mask=mask, output=arr_copy)
...
def test_masked_store_array_indexing():
    ...
    print_op_details("Store Test 5: Array indexing", "store", arr, (indices,), values=values, mask=mask, output=arr_copy)
...
def test_masked_store_mixed_indexing():
    ...
    print_op_details("Store Test 6: Mixed indexing types", "store", arr, (slice(0, 4), 2), values=values, mask=mask, output=arr_copy)
...
def test_masked_store_complex_indexing():
    ...
    print("Store Test 7: Complex multi-dimensional indexing (omitted details, check code to see inputs/outputs)")
...
def test_masked_store_oob_with_true_mask():
    ...
    print_op_details("Store Test 8: OOB with mask=True should raise IndexError", "store", arr, (slice(0, 3), slice(0, 2)), values=values, mask=mask, error_expected="IndexError")
...
def test_masked_store_values_mask_shape_mismatch():
    ...
    print_op_details("Store Test 9: Values and mask shape mismatch should raise AssertionError", "store", arr, (slice(0, 3),), values=values, mask=mask, error_expected="AssertionError")
...
def test_masked_store_values_shape_mismatch():
    ...
    print_op_details("Store Test 10: Values shape mismatch", "store", arr, (slice(0, 3),), values=values, mask=mask, error_expected="IndexError")

...
from triton_viz.sanitizer.symbolic_execution.addptr_overrider import AddPtrSymbolicExecOverrider
...
def test_triple_indirect_load():
    ...
    idx1 = torch.arange(n_elements, dtype=torch.int32, device=device)
    idx2 = torch.randint(0, n_elements, (n_elements,), dtype=torch.int32, device=device)
    offsets = torch.randint(0, n_elements, (n_elements,), dtype=torch.int32, device=device)
    output = torch.zeros_like(idx1)
...
def test_dual_offset_load():
    ...
    offset_a = torch.randint(0, n_elements, (n_elements,), dtype=torch.int32, device=device)
    offset_b = torch.randint(0, n_elements, (n_elements,), dtype=torch.int32, device=device)

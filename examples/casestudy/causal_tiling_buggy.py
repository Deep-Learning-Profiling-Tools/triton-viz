...
START_M = 1  # block row that exhibits the bug


@triton_viz.trace(clients=Tracer())
@triton.jit
def causal_access_buggy(
    scores_ptr,
    out_ptr,
    stride_scores,
    start_m,
    ...
    is_bug_tile = (pid_m == start_m) & (pid_n == start_m)
    mask = tl.where(is_bug_tile, m_mask[:, None] & n_mask[None, :], mask)
...
def run_demo():
    ...
    out_bug = torch.zeros(N_CTX, dtype=torch.float32)

    grid = (triton.cdiv(N_CTX, BLOCK_M), triton.cdiv(N_CTX, BLOCK_N))
    causal_access_buggy[grid](
        scores,
        out_bug,
        scores.stride(0),
        START_M,
    ...
    print("buggy sum (stage1):", out_bug.sum().item())

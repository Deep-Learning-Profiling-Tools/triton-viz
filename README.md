...
- `SANITIZER_ENABLE_FAKE_TENSOR` (default: unset = `0`): three-state tensor materialization. `0` or unset = force real (always copy tensors to CPU upfront). `auto` = lazy auto (fake tensors with lazy materialization; pre-materialises all storages on unmappable pointers before retrying). `1` = force fake (lazy materialization; unmappable pointers raise `UnmappablePointerError`).

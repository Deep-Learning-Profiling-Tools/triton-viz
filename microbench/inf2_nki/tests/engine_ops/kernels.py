...
def tensor_matmul_tiled_factory(
    *,
    ...
    Tilebench's ``matmul_fp32_fp16_fp8`` kernel remains the holdout.
    """
    kernel_dtype_name = dtype_name
    tile_m = tile_k = 128
    tile_n = 512
...
def tensor_matmul_small_factory(
    *,
    ...
    """Attention-sized single-tile TensorE calibration control.

    This is deliberately not the attention holdout: it measures independent
    ``nc_matmul`` instructions with the same small stationary/moving free
    dimensions (M, N <= 128, K <= 128) that single-head attention uses, but
    there is no QK^T/softmax/PV dataflow, no cross-tile chain between the
    dots, and every dot writes a distinct output.  ``repeat`` varies the number
    of retained dots per kernel so the per-dot TensorE slope and the
    once-per-kernel intercept can be separated without operator-specific keys.
    """
    kernel_dtype_name = dtype_name

...
def analyze_records(records):
    ...
    for record in records:
        if isinstance(record, Launch):
            ...
            record_data += [[op_type, count] for op_type, count in op_type_counts.items()]
        elif isinstance(record, LoadStoreBytes):
            def calculate_ratio(record):
                return (record.total_bytes_true / record.total_bytes_attempted
                        if record.total_bytes_attempted > 0 else 0)
            if record.type == "load":
                total_load_bytes_true = record.total_bytes_true
                overall_load_ratio = calculate_ratio(record)
                record_data.append(["Total number of bytes loaded", total_load_bytes_true])
                ...
                record_data.append(["Total number of bytes stored", total_store_bytes_true])
                record_data.append(["Masked Store Ratio", round(overall_store_ratio, 3)])
    if record_data is not None:
        data.append(record_data)

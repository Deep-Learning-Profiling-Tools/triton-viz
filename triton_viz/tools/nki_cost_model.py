...
class DmaCalibrationSurface:
    ...
    def from_csv(cls, path: str | Path,
        ...
        with Path(path).open(encoding="utf-8", newline="") as file:
            for row in csv.DictReader(file):
                ...
                try:
                    ...
                    continue
                if key[0] <= 0 or key[1] <= 0 or not math.isfinite(bandwidth) or bandwidth <= 0:
                    continue
        ...
        if not points:
            raise ValueError(
                f"No {benchmark_name} calibration rows with {bandwidth_column} in {path}"
            )
        return cls(points)
    ...
    def bandwidth_gbps(self, partitions: int, free_bytes: int) -> float:
        ...
        nearest = sorted(
            self.points.items(),
            key=lambda item: (math.log2(item[0][0]) - target_p) ** 2 + (math.log2(item[0][1]) - target_f) ** 2,
        )[:4]
        weighted = 0.0
        weight_sum = 0.0
...
class StaticDmaCalibrationSurface:
    ...
    def latency_ns(self, partitions: int, x: int, y: int) -> float:
        ...
        bounds = tuple(
            (min(point[axis] for point in self.points), max(point[axis] for point in self.points))
            for axis in range(3)
        )
        target = (
        ...
        nearest = sorted(
            self.points.items(),
            key=lambda item: sum(
                (math.log2(item[0][axis]) - log_target[axis]) ** 2
                for axis in range(3)
            ),
        )[:8]
        weighted = 0.0
...
class LoweringExpansionCalibration:
        ...
        with Path(path).open(encoding="utf-8", newline="") as file:
            for row in csv.DictReader(file):
                ...
                if key[0] and key[2] and key[3] > 0 and value[0] > 0:
                    points[key] = value
                    try:
                        fixed_points[key] = max(0.0, float(row.get("kernel_control_active_ns") or 0))
    ...
    def fixed_ns(self, signature: str, dtype: str, engine: str, free_dim: int) -> float:
        ...
        rows = [(free, value) for (sig, calibrated_dtype, calibrated_engine, free), value
                in self.fixed_points.items()
                if sig == signature and calibrated_dtype == dtype and calibrated_engine == engine]
        if not rows:
            return 0.0
        exact = next((value for free, value in rows if free == free_dim), None)
        return exact if exact is not None else min(rows, key=lambda row: abs(row[0] - free_dim))[1]

    def expansions(self, signature: str, dtype: str, free_dim: int,
                   pattern: str | None = None) -> dict[str, tuple[float, int]]:
        ...
        for (sig, calibrated_dtype, engine, calibrated_free), value in self.points.items():
        ...
        for engine in engines:
            rows = by_dtype.get((engine, dtype)) or by_dtype.get((engine, "float32")) or []
            ...
            lower = max((row for row in rows if row[0] <= target), key=lambda row: row[0])
            upper = min((row for row in rows if row[0] >= target), key=lambda row: row[0])
            if lower[0] == upper[0]:
...
class CompositionalLoweringCalibration:
    ...
    def from_csv(cls, path: str | Path) -> "CompositionalLoweringCalibration":
        ...
        with Path(path).open(encoding="utf-8", newline="") as file:
            for row in csv.DictReader(file):
                key = (str(row["engine"]), ComputeCalibration._norm_dtype(row["dtype"]), str(row["target"]))
                coefficients.setdefault(key, {})[str(row["feature"])] = float(row["coefficient"])
        if not coefficients:
    ...
    def predict(self, region_ir: dict[str, Any]) -> dict[str, tuple[float, int, float]]:
        ...
        features = compositional_features(region_ir); dtype = ComputeCalibration._norm_dtype(region_ir.get("dtype"))
        result = {}
        for engine, streams in (("vector", 2), ("scalar", 1)):
            def value(target):
                weights = self.coefficients.get((engine, dtype, target))
                if not weights: return 0.0
                return max(0.0, sum(weights.get(name, 0.0) * amount for name, amount in features.items()))
            count = value("effective_count")
            if count > 0: result[engine] = (count, streams, value("fixed_ns"))
        return result
...
class StructuredControlCalibration:
    ...
    def from_csv(cls, path: str | Path) -> "StructuredControlCalibration":
        ...
        with Path(path).open(encoding="utf-8", newline="") as file:
            for row in csv.DictReader(file):
                key = (row["family"], row["engine"], ComputeCalibration._norm_dtype(row["dtype"]))
                points.setdefault(key, []).append((int(row["free_dim"]), float(row["effective_count"]),
                                                   int(row["instruction_count"]), float(row["fixed_ns"])))
        return cls(points)

    def predict_points(self, region_ir: dict[str, Any]) -> dict[str, tuple[float, int, float]]:
        ...
        family, dtype = structural_family(region_ir), ComputeCalibration._norm_dtype(region_ir.get("dtype"))
        free = int(region_ir.get("free_dim") or region_ir.get("logical_free_dim") or 1); result = {}
        for engine, streams in (("vector", 2), ("scalar", 1)):
            rows = sorted(self.points.get((family, engine, dtype), []))
            if not rows and "__" in family:
                rows = sorted(self.points.get((family.split("__", 1)[0], engine, dtype), []))
            if not rows: continue
            exact = [row for row in rows if row[0] == free]
            if exact:
                row = exact[0]
            else:
                lower = max((row for row in rows if row[0] <= free), default=rows[0], key=lambda x: x[0])
                upper = min((row for row in rows if row[0] >= free), default=rows[-1], key=lambda x: x[0])
                if lower[0] == upper[0]: row = lower
                else:
                    weight = (math.log2(free)-math.log2(lower[0]))/(math.log2(upper[0])-math.log2(lower[0]))
                    row = (free, lower[1]+weight*(upper[1]-lower[1]), round(lower[2]+weight*(upper[2]-lower[2])),
                           lower[3]+weight*(upper[3]-lower[3]))
            if row[1] > 0: result[engine] = (row[1], row[2], row[3])
        return result


    def predict(self, region_ir: dict[str, Any]) -> dict[str, tuple[float, int, float]]:
        points = self.predict_points(region_ir)
        return {engine: (value[0], 2 if engine == "vector" else 1, value[2])
...
def _input_stream_count(event):
    ...
    two_input = {"add", "subtract", "multiply", "divide", "maximum", "minimum",
                 "greater", "less", "tensor_tensor", "where"}
...
class CostModel:
    ...
    def _dma_cost_ns(self, event: dict[str, Any], nbytes: int) -> float:
        ...
        elif str(event.get("mem_src", "")).lower() == "sbuf" and str(
            event.get("mem_dst", "")
        ).lower() == "hbm":
...
class SimulationResult:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "engine_utilization": {
                k: (round(v / self.predicted_latency_ns, 4) if self.predicted_latency_ns else 0.0)
...
def _expand_lowering_groups(
    ...
    source = list(events)
    if (model.lowering_calibration is None and model.compositional_lowering is None
            and model.structured_control_lowering is None) or model.compute_calibration is None:
        return source
    ...
    while index < len(source):
        ...
        dtype = str(
            next(
                (member.get("output_dtype") for member in members if member.get("output_dtype")),
                "float32",
            )
        )
        targets = (model.lowering_calibration.expansions(
            signature, dtype, free_dim, str(event.get("fusion_pattern") or "")
        ) if model.lowering_calibration is not None else {})
        structured = {}
        if not targets and model.compositional_lowering is not None and event.get("region_ir"):
            structured = model.compositional_lowering.predict(event["region_ir"])
            targets = {engine: (value[0], value[1]) for engine, value in structured.items()}
        if not targets and model.structured_control_lowering is not None and event.get("region_ir"):
            structured = model.structured_control_lowering.predict(event["region_ir"])
            targets = {engine: (value[0], value[1]) for engine, value in structured.items()}
        if not targets:
        ...
        for target_index, (engine, (count, streams)) in enumerate(sorted(targets.items())):
            ...
            lowered.update(
                {
                    ...
                    "engine": engine,
                    "input_shape": [1, free_dim],
                    "output_shape": [1, free_dim],
                    "free_dim": free_dim,
                    "output_dtype": dtype,
                    ...
                    "lowering_fixed_ns": (
                        model.lowering_calibration.fixed_ns(signature, dtype, engine, free_dim)
                        if model.lowering_calibration is not None and not structured
                        else (structured.get(engine, (0.0, streams, 0.0))[2]
                              if int(group) == 0 else 0.0)
                    ),
                    ...
                    "output_ptr": final_output if target_index == len(targets) - 1 else None,
                }
            )
...
def simulate(events: Iterable[dict[str, Any]], cost_model: CostModel | None = None) -> SimulationResult:
    ...
    def _dep(earliest: float, producer_end: float, producer_engine: str,
             consumer_engine: str) -> float:
    ...
    for event in events:
        ...
        timeline.setdefault(engine, []).append(
            TimelineEntry(seq=int(event.get("seq", -1)), op=op, engine=engine, start=start, end=end)
        )
        engine_busy[engine] = engine_busy.get(engine, 0.0) + duration
        makespan = max(makespan, end)

    return SimulationResult(
        predicted_latency_ns=makespan + max(0.0, model.kernel_overhead_ns),
        timeline=timeline,
        engine_busy_ns=engine_busy,
    )


def simulate_jsonl(path: str | Path, cost_model: CostModel | None = None) -> SimulationResult:
...
def main() -> None:  # pragma: no cover - thin CLI wrapper
    ...
    parser.add_argument("trace", help="Path to a JSONL trace produced by nki_trace_dump")
    ...
    calibration = DmaCalibrationSurface.from_csv(args.dma_calibration_csv) if args.dma_calibration_csv else None
    write_calibration = (
    ...
    transpose_calibration = (
        DmaCalibrationSurface.from_csv(args.dma_transpose_calibration_csv, "dma_transpose_surface")
        if args.dma_transpose_calibration_csv else None
    )

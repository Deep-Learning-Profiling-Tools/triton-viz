...
class Builder:
    ...
    def load_transpose2d(
        ...
        # Build the SBUF tile directly instead of calling ``self.load``: the
        # frontend records this operation as a single ``LoadTranspose`` event,
        # so an inner ``Load`` record would double-count the same DMA transfer.
        mask_value = getattr(mask, "data", mask) if mask is not None else None
        numpy_keys = self._convert_keys_to_numpy(keys)
        loaded = NDArray(
            value=masked_load(src.data, numpy_keys, mask=mask_value),
            name=f"{src.name}_masked_load",
            buffer=nl.sbuf,
        )
        value = loaded.data.astype(dtype) if dtype is not None else loaded.data
        # The PF-transposed view aliases the same SBUF allocation as the load,
        # so downstream Dot/compute hazards resolve against the recorded
        # destination storage instead of an invented second allocation.
        return NDArray(
            value=value.T,
            name=f"{src.name}_load_transpose2d",
            _storage_state=loaded._storage_state,
        )

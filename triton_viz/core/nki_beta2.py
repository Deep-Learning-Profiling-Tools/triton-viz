...
try:
    ...
    raise ModuleNotFoundError(
        "NKI dependencies are missing. Install triton-viz[nki] to enable the NKI Beta 2 interpreter."
    ) from exc
...
def nc_matmul(
    ...
    # NOTE: this isn't exact nc_matmul semantics, real one overwrites psum
    # if the psum hasn't been written to yet, then accumulates
    dst.data += casted
    return dst
...
def _wrap_result(result, *operands):
    """Wrap NumPy array outputs as NDArray when any operand is NDArray."""
...
def tensor_reduce(dst, op, data, axis=None, keepdims=False, engine=None, name=None):
    """Reduce a tensor along axes using an op compatible with add/max/min."""
    del engine, name
    ...
    if spec is not None:
        exp_bits, mant_bits, finite_only = spec
        casted = _quantize_binary_float(
            reduced, exp_bits, mant_bits, finite_only=finite_only
        )
        casted = np.asarray(casted, dtype=np.float32)
    else:
        casted = np.asarray(reduced, dtype=dst.dtype)
    dst.data[...] = casted
    return dst
...
class Builder:
    ...
    def set_grid_dim(self, *grid_dims):
        """Update the active grid dimensions for interpreter execution."""
...
def num_programs(axes=None):
    """Return launch size along one or more axes."""
    if axes is None:

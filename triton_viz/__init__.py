...
try:
    # ``version.py`` is generated by ``setup.py``/``pip install``. Fall back to a
    # source-tree default so running directly from a checkout (e.g. the NKI
    # microbench and cost-model tools via ``PYTHONPATH=.``) does not require a
    # build step first.

...
export function createTensor(shape, coords, color, tensorName, cubeGeometry, _edgesGeometry = null, _lineMaterial = null) {
    ...
    if (isDense) {
        ...
        if (coords && coords.length > 0) {
            ...
            samples.forEach(c => { if (c[0] > maxIncoming[0])
                maxIncoming[0] = c[0]; if (c[1] > maxIncoming[1])
                maxIncoming[1] = c[1]; if (c[2] > maxIncoming[2])
                maxIncoming[2] = c[2]; });
            const target = [width - 1, height - 1, depth - 1];
            const perms = [[0, 1, 2], [0, 2, 1], [1, 0, 2], [1, 2, 0], [2, 0, 1], [2, 1, 0]];
            const score = (p) => { let s = 0; for (let i = 0; i < 3; i++) {
                s += Math.abs(maxIncoming[i] - target[p[i]]);
                if (maxIncoming[i] > target[p[i]])
                    s += 1000;
            } return s; };
            let bestScore = score(best);
            perms.forEach(p => { let sc = score(p); if (sc < bestScore) {
                best = p;
                bestScore = sc;
            } });

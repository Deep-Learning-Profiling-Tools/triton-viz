...
export function createFlip3D(containerElement, options) {
    ...
    for (let i=0;i<length;i++){
        ...
        cube.position.set(i*(CUBE_SIZE+GAP), current_y, 0);
        initPanel.add(cube);
    }
    const initLabel = createTextSprite('initial array');
    initLabel.position.set(-CUBE_SIZE*3.2, current_y + CUBE_SIZE*0.5, 0);
    ...
    for (let s=0; s<steps.length; s++){
        ...
        const viewPanel = buildPanel(arr, seg, current_y, `step ${s} view`, viewX);
        const swapped = swapBlocksArray(arr, seg);
        const swapPanel = buildPanel(swapped, seg, current_y, `step ${s} swap`, swapX);
    ...
    finalLabel.position.set(-CUBE_SIZE*3.2, current_y + CUBE_SIZE*0.5, 0);

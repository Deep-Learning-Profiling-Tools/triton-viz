...
export function createStoreVisualization(containerElement, op) {
    ...
    const COLOR_LEFT_SLICE = new THREE.Color(0.0, 1.0, 0.0); // Bright Green (starting color for left slice)
    const COLOR_STORED = new THREE.Color(1.0, 0.0, 1.0);    // Magenta (final color for left slice)
    const COLOR_LOADED = new THREE.Color(1.0, 0.8, 0.0);    // Gold (final color for global tensor)
    ...
    function animate() {
        ...
        if (frame < op['slice_shape'][0] * op['slice_shape'][1] * 2) {
            const i = Math.floor(frame / 2 / op['slice_shape'][1]);
            const j = Math.floor(frame / 2) % op['slice_shape'][1];
            ...
            leftSliceTensor.children[leftSliceIndex].material.color.copy(
                interpolateColor(COLOR_LEFT_SLICE, COLOR_STORED, factor)
            );

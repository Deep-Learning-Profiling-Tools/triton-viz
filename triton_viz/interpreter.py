...
class Builder:
    ...
    def sort_tensor_handles(self):
        ...
        launch_record.tensors = sorted(launch_record.tensors, key=lambda x: x.ptr)

    # from a give ptr, get where the original tensor is stored
    def get_tensor_ptr(self, ptr):
        # tensors are sorted by ptr
        ret_idx = 0
        for i in range(len(self.launch_records[-1].tensors)):
            if ptr < self.launch_records[-1].tensors[i].ptr:
                break
            ret_idx = i
        return self.launch_records[-1].tensors[ret_idx]
    ...
    def set_grid_idx(self, x, y, z):
        ...
        self.grid_idx = (x, y, z)
        grid_record = GridRecord(self.grid_idx)
        self.add_record(grid_record)

    def set_grid_dim(self, nx, ny, nz):
        self.grid_dim = (nx, ny, nz)
        self.launch_records.append(LaunchRecord((nx, ny, nz), [], []))
    ...
    def __call__(self, *args_dev, **kwargs):
        ...
        for _, arg in args.items():
            if isinstance(arg, tl.tensor):
                builder.set_tensor_handle(arg.handle)
        builder.sort_tensor_handles()
...
class InterpretedFunction:
    def __init__(self, fn) -> None:
        ...
        self._arg_names = [v.name for v in signature.parameters.values()]
